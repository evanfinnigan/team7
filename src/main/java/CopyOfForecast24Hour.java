...
public class CopyOfForecast24Hour extends TransparentPanel {
	...
	public CopyOfForecast24Hour(WeatherData data, WeatherPreferences p) {
		this.data24h = data;
		this.preferences = p;
		initComponents();
		createDisplay();
	...
	private void createDisplay() {
		...
		for (int i = 0; i < 8; i++) {
			...
			//	block.add(temp_l[i]);
		//	block.add(image_l[i]);
		//	block.add(skycondition_l[i]);
		//	block.add(weatherdescription_l[i]);
